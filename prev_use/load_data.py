...
def admin_related_clean(text,keywords):
    paragraph=text.split("\n\n")
    kept=[]
    for p in paragraph:
        hits=sum(term in p.lower() for term in keywords)
        if(hits>=2):
            continue
        kept.append(p)
    return ("\n\n").join(kept)
...
text = ""
pages=[]
with pdfplumber.open("troubleshoot-microsoftteams.pdf") as pdf:
...
# Save extracted text
keep_keywords = [
    "troubleshoot", "fix", "resolve",
    "issue", "error", "not working", "cannot"
]

remove_keywords = [
    "powershell", "tenant", "policy",
    "admin center", "deployment", "licensing"
]
...
filtered_pages = []
for p in pages:
        filtered_p=admin_related_clean(p,remove_keywords)
        filtered_p=clean_page_safe(filtered_p,line_counter)
...
# merging pages to get proper page windows so that steps are not left broken
...
windowed_pages = merge_page_windows(filtered_pages, window=1)
print(len(windowed_pages))
# saving the pages after filtering
with open("pagewise_filtered_troubleshoot.txt","w",encoding="utf-8") as f:
    i=1
    for page in filtered_pages:
        f.write(f"\n----------PAGE {i}---------------\n")
        f.write(page)
        i+=1
# saving the cleaned windows
with open("teams_cleaned_windowed.txt","w",encoding="utf-8") as f:
    i=1
    for p in windowed_pages:
        f.write(f"\n----------PAGE {i}--------\n")
        f.write(p)
        i+=1