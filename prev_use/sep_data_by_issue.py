...
ISSUE_REGEX = re.compile(
    r"^(troubleshoot|fix|resolve)\s+.+"
    r"|^microsoft teams.+(not|cannot|won't|fails)"
    r"|^.+(issues|problems)\s+in\s+microsoft teams",
    re.IGNORECASE
)

def load_windowed_pages(path):
    pages = []
    current = None
...
def extract_issue_title(raw_title):
    ...
    title = re.sub(r"in microsoft teams", "", title)
...
def dedupe_preserve_order(lines): # to remove lines when they are repeated in issues
    ...
    for line in lines:
        ...
        if key not in seen:
            ...
            unique.append(line)
    return unique
pages = load_windowed_pages("teams_cleaned_windowed.txt")
print("Windowed pages loaded:", len(pages))
issues = []
current_issue = None
...
for issue in issues:
    ...
    if clean_title not in merged_issues:
        ...
        merged_issues[clean_title] = {
            ...
            "content": [],
            "document_title":"troubleshoot-microsoftteams.pdf",
...
'''for issue in final_issues:
    print("-",issue["document_title"])'''
# saving the issues
with open("issues.json", "w", encoding="utf-8") as f:
    print(final_issues[0]["document_title"])
